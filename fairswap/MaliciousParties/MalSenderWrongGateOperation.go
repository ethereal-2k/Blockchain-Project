...
func main(){
		...
		//go routines
		go maliciousSender_WrongGateOperation(authSender,client,channel_SenToRec,channel_SenToMain)
...
//maliciousSender_WrongGateOperation function
//In this function the sender calculates a wrong output of a gate
//Input parameters:
...
func maliciousSender_WrongGateOperation(authSender *bind.TransactOpts,client *backends.SimulatedBackend,channel_SenToRec chan []byte,channel_SenToMain chan string){
	...
	/*malicious act*/
	//the sender sets a wrong circuit and calulates the wrong encodedGateOutputs
	var maliciousCircuitObjects [totalNumberOfGates]circuit
	//sets the malicious circuit
	setMaliciousCircuitTuples(&maliciousCircuitObjects)
	maliciousEncodedGateOutputs,_:=Encode(inputVectors,key,maliciousCircuitObjects,MRinp)

	/*SenToRecObject initialize*/
	idInitializingForRec(&SenToRecObject)
	SenToRecObject.Keycommit=keyCommit
	SenToRecObject.EncodedOutputOfGates=maliciousEncodedGateOutputs
